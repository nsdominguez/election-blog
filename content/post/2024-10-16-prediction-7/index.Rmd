...
g1 <- ggplot(data = map) +
  ...
  geom_sf_text(aes(label = stateab,
                   text = paste(
                     google_name,
                     '<br>Predicted 2-P Harris Vote:', prediction_mod1[,"fit"],'%',
  ...
  labs(title = "2024 Presidential Forecast Model - Ground Game",
...
g2 <- ggplot(test_viz1, aes(x = prediction_mod1[,"fit"], y = reorder(state_full, prediction_mod1[,"fit"]), 
  ...
  geom_col(
    aes(text = paste(
             state_full,
             '<br>Predicted 2-P Harris:', prediction_mod1[,"fit"],'%',
...
FEC_predictions <- read_csv("FEC_predictions1.csv")
```
...
g3 <- ggplot(data = map2) +
  ...
  geom_sf_text(aes(label = iso3166_2,
                   text = paste(
                     google_name,
                     '<br>Predicted 2-P Harris Vote:', final_prediction,'%',
  ...
  scale_fill_gradient2(low = "red", mid = "white", high = "navyblue", midpoint = 50, 
                       ...
                       limits = c(20, 80),
                       name = "Harris (D) Predicted Two-Party Vote Share*") +
  labs(title = "2024 Presidential Election Predictions: Ground Game Ensemble Model") + 
  theme_void() +
  theme(legend.position = "none") + 
  theme(plot.title = element_text(face = "bold"))
...
This ensemble model predicts that Republican former president Donald Trump will win the 2024 election with **278** electoral votes, compared to Democrat Vice President Kamala Harris’ **260** electoral votes; an extremely close election. But, the prediction becomes even closer when you consider that the model predicts Harris will get 49.99456 percent of the two-party vote share! If my model is off by a fraction of a decimal point, then Harris could win the 2024 presidential election with exactly 270 electoral votes! It literally does not get any closer than this. In addition to Wisconsin, the model predicts that the six other swing states will also be extremely close calls, indicating that the election is still any candidate’s game. 
...
g4 <- ggplot(final_combined_predictions, aes(x = final_prediction, y = reorder(state, final_prediction), 
  ...
  geom_col(
    aes(text = paste(
             state,
             '<br>Predicted 2-P Harris:', final_prediction,'%',
  ...
  labs(x = "Harris (D) Predicted Two-Party Vote Share",
       y = "State",
       title = "Ground Game Ensemble Model Predictions") +
  geom_vline(xintercept = 50, linetype = "dotted") +
  theme(panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0, size = 14),
        axis.text.y = element_text(size = 5),
