...
# TODO: Model 1 = 0.1881 , Model 2 = 0.3248

res <- resid(reg_econ)
plot(fitted(reg_econ), res)
abline(0,0)
...
# Model Testing: Leave-One-Out - -18.97913 (very bad!)
(out_samp_pred <- predict(reg_econ_2, d_inc_econ[d_inc_econ$year == 2020,])) #train model without 2020
(out_samp_truth <- d_inc_econ |> filter(year == 2020) |> select(pv2p)) #then use model to predict 2020
out_samp_pred - out_samp_truth # Dangers of fundamentals-only model! 
#model predicts trump gets 28% of the vote, 18 pp less than actual
# https://www.nytimes.com/2020/07/30/business/economy/q2-gdp-coronavirus-economy.html

# Model Testing: Cross-Validation (One Run) - -0.07052 (much better) 
...
# Model Testing: Cross-Validation (1000 Runs) - -0.108
...
# predicted incumbent vote share 51.58486
...
# predicted 51.58486 (41.86, 61.31)
# if this is too large, forecasters decrease confidence level for a narrower interval

# Achen and Bartels argue voters are not rational and base their retrospective voting on economic performance in the last few months before the election. 
# do extension 3 for electoral college predicition and use state data. 
# not necessary to combine last and this week's models.
